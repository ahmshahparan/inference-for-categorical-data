...
These percentages appear to be sample statistics (derived from the data sample) or population parameters because they are generated from results of the survey.
...
* Sample size is large enough and one sample is good enough.
...
Each row in table 6 corresponds to results of the survey for each country. Each row of `atheism` corresponds to one observation, claiming to either be athiest or non atheist, complete with year.
...
us12Atheism <- subset(atheism, nationality == "United States" & year == "2012" & response == "atheist")
nrow(us12Atheism)/nrow(us12)
```
...
```
```{r}
inference(spain12$response, est="proportion",type="ci",success = "atheist" ,method="theoretical")
```
z value for 95% confidence will be z=1.96</br>
...
```
```{r}
inference(italy12$response, est="proportion",type="ci",success = "atheist" ,method="theoretical")
```
z value for 95% confidence will be z=1.96</br>
Standard error: SE = 0.0086
```{r}
z <- 1.96
SE <- 0.0086
MoE <- z * SE
MoE
```
The conditions for the inference for Spain and Italy are met.

The sample sizes are much less than 10% of the total populations for each country and the sample sizes are big enough to satisfy the success-failure condition.
...
The relation between `p` and `me` is parabolic. me increases with increasing P untill it reaches a maximum at 0.5 (50%) and then descreases as P increases to 1.
...
The sampling distribution has a near normal distribution with the mean close to the population mean of 0.1.
...
hist(p_hats2, main = "p = 0.2, n = 1040", xlim = c(0.14, 0.30))
...
hist(p_hats3, main = "p = 0.2, n = 400", xlim = c(0.14, 0.30))
```
```{r}
par(mfrow = c(1, 1))
```
...
**null Hypothesis :** There is no difference in the averages of atheism, in spain from 2005 to 2012</br>
...
```
Computing the Standard Error (SE) for the difference of two proportions -
```{r}
p_spn05 = 0.1003
n_spn05 = 1146 
p_spn12 = 0.09
...
```
```{r}
#Confidence interval for difference between proportion in 2005 and 2012
PE_spn + (1.96*SE_spn)
PE_spn - (1.96*SE_spn)
```
Since the confidence interval (95%) overlap, there is no convincing evidence that Spain has seen a change in its atheism index between 2005 and 2012.

  **b.** Is there convincing evidence that the United States has seen a
    change in its atheism index between 2005 and 2012?
    
    
**null Hypothesis :** There is no difference in the averages of atheism, in USA from 2005 to 2012</br>
**alternative Hypothesis :** There is difference in the averages of atheism, in USA from 2005 to 2012
```{r}
...
PE_usa - (1.96*SE_usa)
```
There is evidence that there is a change in the atheism index in the USA from 2005 to 2012. The confidence interval does NOT include 0 so we reject the null hypohtesis that 2005 to 2012 is zero.
...
We would expect to see about 2 ( 0.05 * 39) type 1 errors out of the 39 countries at a .05 level of significance.
...
me <- 0.01 # Marging of error
n <- p * (1 - p) * z ^ 2  / me ^ 2
n
```
And based on the results we will need at least 9604 people to ensure the sample proportion is within guidelines.
